...
function buildSessionList(all: ChatHistoryDto[]): SessionItem[] {
  ...
  for (const [memoryId, arr] of byMem.entries()) {
    const latest = arr.reduce((best, cur) =>
      safeTimeMs(cur.createTime) > safeTimeMs(best.createTime) ? cur : best
    );
...
export default function App() {
  ...
  const appendAssistantChunk = useCallback((assistantId: string, chunk: string) => {
    setMessages((prev) =>
      prev.map((m) =>
        m.id === assistantId ? { ...m, content: m.content + chunk, error: false } : m
      )
    );
  }, []);
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      const msg =
        e instanceof Error ? e.message : `请求异常：${String(e)}`;
          ...
          <div className="session-list" aria-busy={sessionsLoading}>
            {sessions.length === 0 && !sessionsLoading ? (
              ...
              sessions.map((s) => (
                <div
                  key={s.memoryId}
                  className={`session-item ${roomId === s.memoryId ? "session-item-active" : ""}`}
                >
                  <button
                    type="button"
          ...
          <div className="chat-scroll" ref={listRef}>
            {messages.length === 0 && (
              <div className="empty-hint">
                在下方输入问题，通过流式回复实时查看 AI 回答。
              </div>
            )}
            {messages.map((m) => (
              <div
                key={m.id}
                className={`row ${m.role === "user" ? "row-user" : "row-ai"}`}
              >
                <div
